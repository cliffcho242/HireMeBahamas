...
async function main() {
  ...
  // Create favicon.ico (proper ICO format with multiple sizes)
  // Note: Sharp doesn't support ICO format directly, so we create a PNG as fallback
  // For production, consider using a dedicated ICO converter or tool
  const faviconBuffer = await createIconBuffer(32, 32);
  await sharp(faviconBuffer)
    .resize(32, 32)
    .png()
    .toFile(path.join(publicDir, 'favicon.ico'));
  console.log('  ✓ favicon.ico (Website Favicon - PNG format)');

  // Create vite.svg (keep the official Vite logo)
  fs.writeFileSync(path.join(publicDir, 'vite.svg'), VITE_LOGO_SVG);
