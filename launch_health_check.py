...
tests = [
    ...
    ('Login', 'POST', 'http://127.0.0.1:8008/api/auth/login', {'email': 'admin@hirebahamas.com', 'password': 'AdminPass123!'}),
...
for test_name, method, url, *data in tests:
    try:
        ...
        if method == 'POST':
            r = requests.post(url, json=data[0] if data else {}, headers=headers, timeout=10)
        else:
