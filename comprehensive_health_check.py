...
def main():
    ...
    success, login_response = test_endpoint('Admin Login', 'POST', f'{BASE_URL}/api/auth/login', login_data)
    if success: tests_passed += 1
    ...
    if login_response and isinstance(login_response, dict):
        ...
        if token:
            ...
            # Profile test
            total_tests += 1
            success, _ = test_endpoint('User Profile', 'GET', f'{BASE_URL}/api/auth/profile', headers=headers)
            if success: tests_passed += 1

            # HireMe available test
            total_tests += 1
            success, hireme_data = test_endpoint('HireMe Available', 'GET', f'{BASE_URL}/api/hireme/available', headers=headers)
            if success:
                tests_passed += 1
