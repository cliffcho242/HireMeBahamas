...
def test_backend_login():
    ...
    try:
        r = requests.post('http://127.0.0.1:8008/api/auth/login',
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            if 'token' in response_data and 'user' in response_data:
                ...
                print(f"❌ Frontend login simulation: INVALID RESPONSE - {response_data}")
                return False
        else:
...
def main():
    ...
    if backend_ok and frontend_ok and login_ok:
        ...
        if not frontend_ok:
            print("   → Frontend connection issue - check if frontend is running on port 3001")
        if not login_ok:
