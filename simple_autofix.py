...
def test_http_endpoint(url):
    ...
    def show_status(self):
        ...
        self.log(f"Backend (:{self.backend_port}): {backend_status}", 
                "SUCCESS" if backend_ok else "ERROR")
        self.log(f"Frontend (:{self.frontend_port}): {frontend_status}", 
                "SUCCESS" if frontend_ok else "ERROR")
                
        if backend_ok and frontend_ok:
            self.log("All services are running!", "SUCCESS")
            self.log(f"Access your app at: http://localhost:{self.frontend_port}", "INFO")
