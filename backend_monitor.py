...
class BackendMonitor:
    ...
    def test_login_continuously(self):
        ...
        while self.monitoring:
            try:
                ...
                print(f"[{time.strftime('%H:%M:%S')}] Login test - Status: {response.status_code}")
    ...
    def test_endpoints(self):
        ...
        endpoints = [
            ("GET", "/health", None),
            ("POST", "/api/auth/login", {"email": "admin@hirebahamas.com", "password": "admin123"}),
            ("GET", "/api/auth/profile", None)  # This will fail without token but we can see the response
        ]
