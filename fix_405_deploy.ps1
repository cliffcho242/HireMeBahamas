...
# Step 2: Check and fix vercel.json configuration
...
# Step 3: Check and fix frontend environment
Write-Host "`n📁 Step 3: Checking frontend environment..." -ForegroundColor Yellow
if (Test-Path "frontend/.env") {
    $envContent = Get-Content "frontend/.env"
    $hasCorrectApiUrl = $false
    
    foreach ($line in $envContent) {
        if ($line -eq "VITE_API_URL=https://hiremebahamas.onrender.com") {
            $hasCorrectApiUrl = $true
            break
        }
    }
