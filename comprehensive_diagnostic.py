...
class SystemDiagnostic:
    ...
    def check_python_environment(self):
        ...
        for package in packages:
            try:
                result = subprocess.run([str(venv_python), "-c", f"import {package.replace('-', '_')}"], 
    ...
    def check_database(self):
        ...
        try:
            ...
            if cursor.fetchone():
                ...
                # Check admin user
                cursor.execute("SELECT email FROM users WHERE email='admin@hirebahamas.com'")
                if cursor.fetchone():
    ...
    def test_backend_start(self):
        ...
        try:
            ...
            if "Serving Flask app" in stdout or "running on" in stdout.lower():
                ...
                self.log_issue(f"Backend start failed. Stdout: {stdout[:200]}... Stderr: {stderr[:200]}...")
                return False
