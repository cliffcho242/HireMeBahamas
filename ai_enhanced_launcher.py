...
class PlatformLauncher:
            ...
            for attempt in range(10):
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        self.log(f"AI Monitor: {health.get('ai_monitor', 'Not available')}", "INFO")
                        return True
                except:
                    time.sleep(2)
    ...
    def test_login_system(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log(f"Admin user: {result.get('user', {}).get('email', 'Unknown')}", "INFO")
                ...
                if error_response.status_code == 401:
                    self.log("Error monitoring active - invalid login detected", "SUCCESS")
    ...
    def launch_platform(self):
        ...
        self.log("Monitor Status: http://127.0.0.1:8008/api/system/monitor/status", "INFO")
        ...
        try:
            while True:
                ...
                try:
                    response = requests.get("http://127.0.0.1:8008/health", timeout=3)
                    if response.status_code != 200:
                        launcher.log("Backend health check failed - may need restart", "WARNING")
                except:
                    launcher.log("Backend not responding - auto-healing should engage", "WARNING")
