...
def test_valid_database_url_structure():
    ...
    with patch.dict(os.environ, {"DATABASE_URL": valid_url}, clear=True):
        ...
        with patch('app.database.create_engine') as mock_create_engine:
            ...
            assert call_args.kwargs['pool_pre_ping'] is True, "pool_pre_ping should be True"
            assert call_args.kwargs['pool_recycle'] == 300, "pool_recycle should be 300"
            assert call_args.kwargs['connect_args']['sslmode'] == 'require', "sslmode should be require"
...
def test_engine_configuration():
    ...
    with patch.dict(os.environ, {"DATABASE_URL": valid_url}, clear=True):
        ...
        with patch('app.database.create_engine') as mock_create_engine:
            ...
            print(f"    - sslmode: {call_args.kwargs.get('connect_args', {}).get('sslmode')}")
            
            assert call_args.kwargs.get('pool_pre_ping') is True, "pool_pre_ping must be True"
            assert call_args.kwargs.get('pool_recycle') == 300, "pool_recycle must be 300"
            assert call_args.kwargs.get('connect_args', {}).get('sslmode') == 'require', "sslmode must be require"
    ...
    with patch.dict(os.environ, {"DATABASE_URL": valid_url}, clear=True):
        ...
        with patch('app.database.create_engine') as mock_create_engine:
            ...
            with patch('logging.info') as mock_info:
                db_module.engine = None
                db_module.init_db()
                mock_info.assert_called_once_with("Database engine initialized successfully")
