...
class PlatformDiagnostics:
    ...
    def check_dependencies(self):
        ...
        required_packages = [
            'flask', 'flask-cors', 'bcrypt', 'PyJWT', 'requests', 'psutil'
        ]
    ...
    def generate_report(self):
        ...
        self.log(f"Backend (port {self.backend_port}): {backend_status}", 
                "SUCCESS" if backend_status == "RUNNING" else "ERROR")
        self.log(f"Frontend (port {self.frontend_port}): {frontend_status}", 
            ...
            try:
                while True:
                    ...
                    if not self.test_backend_health() and self.backend_process:
                        self.log("Backend health check failed, restarting...", "WARNING")
                        self.start_backend_server()
                    if not self.test_frontend_health() and self.frontend_process:
                        self.log("Frontend health check failed, restarting...", "WARNING")
