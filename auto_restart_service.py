...
class ServerMonitor:
    ...
    def check_backend(self):
        ...
        try:
            response = requests.get(
                f"http://127.0.0.1:{BACKEND_PORT}/health", timeout=5
            )
    ...
    def restart_backend(self):
        ...
        if self.backend_restart_count >= MAX_RESTART_ATTEMPTS:
            self.log(
                f"❌ Max restart attempts ({MAX_RESTART_ATTEMPTS}) reached for backend",
                "ERROR",
            )
            return False
        ...
        self.log(
            f"Restarting backend (attempt {self.backend_restart_count}/{MAX_RESTART_ATTEMPTS})...",
            "WARNING",
        )
        self.stop_backend()
        time.sleep(2)
    ...
    def restart_frontend(self):
        ...
        if self.frontend_restart_count >= MAX_RESTART_ATTEMPTS:
            self.log(
                f"❌ Max restart attempts ({MAX_RESTART_ATTEMPTS}) reached for frontend",
                "ERROR",
            )
            return False
        ...
        self.log(
            f"Restarting frontend (attempt {self.frontend_restart_count}/{MAX_RESTART_ATTEMPTS})...",
            "WARNING",
        )
        self.stop_frontend()
        time.sleep(2)
