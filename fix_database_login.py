...
def fix_database_and_create_admin():
    ...
    if "password_hash" not in column_names:
        ...
        for user in existing_users:
            if (
                len(user) >= 4 and user[1] != "admin@hirebahamas.com"
            ):  # Skip admin, we'll recreate
                try:
                    cursor.execute(
                        """
                    ...
                    """,
                        (
                            user[1],
                            user[2] if len(user) > 2 else f"user_{user[0]}",
...
def test_login_direct():
    ...
    cursor.execute(
        "SELECT id, password_hash, username FROM users WHERE email = ?", (email,)
    )
