...
try:
    from ai_system_monitor import start_ai_monitor, get_system_health
...
class AIUserAnalytics:
    ...
    def track_user_interaction(self, user_id: str, action: str, content_id: str = None, 
    ...
    def _calculate_engagement_score(self, df: pd.DataFrame) -> float:
        ...
        engagement_score = min((total_score / max_possible) * 100, 100) if max_possible > 0 else 0
    ...
    def get_content_recommendations(self, user_id: str, limit: int = 10) -> List[Dict]:
        ...
        if user_type == 'content_creator':
            recommendations.extend(self._get_creator_recommendations(user_id, limit // 2))
        elif user_type == 'active_commenter':
            recommendations.extend(self._get_discussion_recommendations(user_id, limit // 2))
        elif user_type == 'social_connector':
            recommendations.extend(self._get_social_recommendations(user_id, limit // 2))
...
class FacebookLikePlatform:
    ...
    def create_admin_user(self):
        ...
        cursor.execute("SELECT id FROM users WHERE email = ?", ("admin@hirebahamas.com",))
    ...
    def setup_routes(self):
        ...
        def health_check():
            return jsonify({
                ...
                'features': ['posts', 'comments', 'likes', 'messaging', 'ai_recommendations']
            })
        ...
        def monitor_status():
            ...
            try:
                status = {
                    ...
                    'performance_history_length': len(self.ai_monitor.performance_history),
                    'last_check': self.ai_monitor.health.last_check.isoformat() if self.ai_monitor.health.last_check else None
                }
        ...
        def login():
            try:
                ...
                if not email or not password:
                    ...
                    if self.ai_monitor:
                        self.ai_monitor.record_error_pattern('login_missing_credentials', datetime.now())
                ...
                if user and bcrypt.checkpw(password.encode('utf-8'), user[1].encode('utf-8')):
                    ...
                    self.ai_analytics.track_user_interaction(
                        str(user[0]), 'login', duration=0,
                        metadata={'timestamp': datetime.now().isoformat()}
                    )
                    
                    # Log successful login to AI monitor
                    if self.ai_monitor:
                        self.ai_monitor.record_error_pattern('login_success', datetime.now())
                    ...
                    if self.ai_monitor:
                        self.ai_monitor.record_error_pattern('login_invalid_credentials', datetime.now())
                    return jsonify({'error': 'Invalid credentials'}), 401
                    
            except sqlite3.Error as e:
                # Database error - critical for AI monitor
                if self.ai_monitor:
                    self.ai_monitor.record_error_pattern('login_database_error', datetime.now())
                return jsonify({'error': 'Database error during login'}), 500
                
            except Exception as e:
                # General error - log to AI monitor
                if self.ai_monitor:
                    self.ai_monitor.record_error_pattern('login_general_error', datetime.now())
        ...
        def get_posts():
            ...
            recommendations = []
            if user_id:
                recommendations = self.ai_analytics.get_content_recommendations(str(user_id))
                
                # Track feed view interaction
                self.ai_analytics.track_user_interaction(
                    str(user_id), 'feed_viewed', 
                    duration=0, metadata={'posts_count': len(posts)}
                )
            
            return jsonify({
                'posts': posts,
                'recommendations': recommendations,
                'ai_insights': self.ai_analytics.analyze_user_patterns(str(user_id)) if user_id else None
            })
        ...
        def create_post():
            ...
            # Track post creation
            self.ai_analytics.track_user_interaction(
                str(user_id), 'post_created', str(post_id),
        ...
        def like_post(post_id):
            ...
            # Track interaction
            self.ai_analytics.track_user_interaction(
                str(user_id), 'like_given' if action == 'like' else 'like_removed',
                str(post_id), duration=1
            )
            
            return jsonify({'action': action, 'message': f'Post {action}d successfully'})
        ...
        def get_user_analytics():
            ...
            return jsonify({
                ...
                'recommendations': recommendations,
                'analytics_summary': {
                    'total_interactions': len(self.ai_analytics.user_interactions.get(str(user_id), [])),
    ...
    def setup_socket_events(self):
        ...
        def handle_typing(data):
            user_id = self.get_current_user_id()
            if user_id:
                emit('user_typing', {'user_id': user_id}, broadcast=True, include_self=False)
