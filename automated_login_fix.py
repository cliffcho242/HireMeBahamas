...
class LoginFixAutomator:
            ...
            try:
                if "login" in endpoint:
                    ...
                    if response.status_code in [200, 401, 422]:
                        ...
                        self.log(f"Endpoint returned {response.status_code}: {endpoint}", "WARNING")
                else:
    ...
    def fix_api_endpoints(self):
        ...
        if backend_file.exists():
            ...
            if "/api/auth/login" not in backend_content and "/auth/login" in backend_content:
                self.log("Found endpoint mismatch - backend has /auth/login but frontend expects /api/auth/login", "WARNING")
                ...
                if "@app.route('/auth/login'" in backend_content:
                    ...
                    login_route_pattern = r"@app\.route\('/auth/login'[^}]+\n    return [^}]+\n}"
                    import re
                    ...
                    if login_func_start != -1:
                        ...
                        new_content = backend_content[:login_func_end] + new_route + backend_content[login_func_end:]
        ...
        for db_path in db_paths:
            if db_path.exists():
                ...
                try:
                    ...
                    # Check admin user
                    cursor.execute("SELECT id, email, password_hash FROM users WHERE email = ?", 
                    ...
                    if not admin_user:
                        ...
                        # Test password hash
                        try:
                            password_check = bcrypt.checkpw("admin123".encode(), admin_user[2].encode())
                            if not password_check:
                                self.log(f"Invalid admin password hash in {db_path}", "ERROR")
                                self.issues_found.append(f"Corrupted admin password: {db_path}")
                                all_good = False
                            else:
                                self.log(f"Admin user OK in {db_path}", "SUCCESS")
                        except Exception as e:
                            self.log(f"Password verification error in {db_path}: {e}", "ERROR")
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if result.get('token'):
                        ...
                        self.log(f"Login response missing token at {endpoint}", "WARNING")
                else:
                    self.log(f"Login failed at {endpoint}: {response.status_code} - {response.text}", "ERROR")
    ...
    def run_automated_fix(self):
        ...
        if login_working:
            ...
            self.log("✅ Admin Credentials: admin@hirebahamas.com / admin123", "SUCCESS")
            ...
            self.log("Please check the errors above and try manual troubleshooting", "ERROR")
