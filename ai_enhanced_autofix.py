...
class SafeLogger:
    ...
    def log(self, message: str, level: str = "INFO"):
        levels = {"INFO": "[INFO]", "SUCCESS": "[OK]", "ERROR": "[ERROR]", "WARNING": "[WARN]", "FIX": "[FIX]"}
        try:
            print(f"{levels.get(level, '[LOG]')} {datetime.now().strftime('%H:%M:%S')} {message}")
            with open(self.log_file, 'a', encoding='utf-8') as f:
                f.write(f"{datetime.now().isoformat()} - {level} - {message}\n")
        except UnicodeEncodeError:
            safe_message = message.encode('ascii', 'ignore').decode('ascii')
            print(f"{levels.get(level, '[LOG]')} {datetime.now().strftime('%H:%M:%S')} {safe_message}")
...
class AdvancedLoginAI:
    ...
    async def smart_diagnosis(self) -> Dict:
        ...
        diagnosis["repair_priority"] = sorted(all_issues, key=lambda x: 
            {"critical": 3, "high": 2, "medium": 1, "low": 0}[x["severity"]], reverse=True)
        
        safe_logger.log(f"Diagnosis complete: {total_issues} issues, confidence: {diagnosis['ai_confidence']:.2f}", 
    ...
    async def check_backend_intelligence(self) -> Dict:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return {"healthy": False, "error": f"Backend returned {response.status_code}"}
        except requests.exceptions.ConnectionError:
            return {"healthy": False, "error": "Backend not responding - server offline"}
    ...
    async def check_database_intelligence(self) -> Dict:
        ...
        try:
            ...
            if missing:
                conn.close()
                return {"healthy": False, "error": f"Missing database columns: {missing}"}
    ...
    async def check_auth_intelligence(self) -> Dict:
        ...
        try:
            ...
            # Test password hash
            try:
                is_valid = bcrypt.checkpw("admin123".encode('utf-8'), admin[1].encode('utf-8'))
                conn.close()
                ...
                conn.close()
                return {"healthy": False, "error": f"Password verification failed: {str(e)}"}
    ...
    async def check_endpoint_intelligence(self) -> Dict:
        ...
        try:
            ...
            if response.status_code in [200, 401, 422, 400]:
                ...
                return {"healthy": False, "error": f"Login endpoint returned {response.status_code}"}
    ...
    async def auto_heal_system(self, diagnosis: Dict) -> bool:
        ...
        for issue in diagnosis["repair_priority"]:
            ...
            safe_logger.log(f"Healing: {issue_type} (severity: {issue['severity']})", "FIX")
            ...
            if fix_strategy in self.auto_heal_strategies:
                try:
                    ...
                    safe_logger.log(f"Healing error for {issue_type}: {str(e)}", "ERROR")
                    healing_success = False
            else:
        ...
        try:
            ...
            if fix_script.exists():
                result = await asyncio.to_thread(subprocess.run,
                    ...
                    capture_output=True, cwd=str(self.workspace_path)
                )
    ...
    async def heal_login_endpoints(self, issue: Dict) -> bool:
        ...
        try:
            ...
            api_files = [
                ...
                self.workspace_path / "frontend" / "src" / "services" / "api_ai_enhanced.ts"
            ]
            ...
            for api_file in api_files:
                if api_file.exists():
                    ...
                    if "'/auth/login'" in content and "'/api/auth/login'" not in content:
    ...
    async def comprehensive_login_test(self) -> Dict:
        ...
        if test_results["overall_success"]:
            safe_logger.log(f"All {test_results['tests_passed']} login tests passed!", "SUCCESS")
        else:
            safe_logger.log(f"Login tests: {test_results['tests_passed']} passed, {test_results['tests_failed']} failed", "WARNING")
        ...
        while self.monitoring_active:
            try:
                ...
                if diagnosis["system_health"] != "optimal":
                    safe_logger.log(f"Issues detected: {len(diagnosis['critical_issues'])} critical, {len(diagnosis['warnings'])} warnings", "WARNING")
                    ...
                    if healing_success:
                        ...
                        if test_result["overall_success"]:
                            safe_logger.log("System fully operational after AI healing", "SUCCESS")
                        else:
                            safe_logger.log("System needs manual intervention", "WARNING")
                    else:
                        safe_logger.log("AI healing failed - manual intervention required", "ERROR")
                else:
                    ...
                    if test_result["overall_success"]:
                        ...
                        safe_logger.log("Tests failing despite healthy diagnosis", "WARNING")
...
async def run_ai_enhanced_login_fix():
    ...
    if success:
        ...
        safe_logger.log("Failed to repair login system - manual intervention needed", "ERROR")
        return False
...
if __name__ == "__main__":
    ...
    def ai_analyze_connection_error(self) -> List[str]:
        ...
        if self.error_patterns['connection_refused']['count'] > 0:
            ...
            if successful_fixes:
                fixes.insert(0, f"AI recommends: {successful_fixes[-1]} (success rate: high)")
    ...
    def ai_analyze_http_error(self, status_code: int) -> List[str]:
        ...
        error_map = {
            404: ['Check API endpoints', 'Verify URL paths', 'Update route definitions'],
            500: ['Check server logs', 'Verify database connection', 'Check Python dependencies'],
    ...
    def test_frontend_connection(self) -> Dict:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"✅ Frontend healthy (response time: {response_time:.3f}s)")
            else:
                result['status'] = 'error'
        ...
        try:
            ...
            for i in range(30):  # Wait up to 30 seconds
                ...
                if self.check_port_availability('127.0.0.1', 8008):
                    ...
                    self.error_patterns['connection_refused']['fixes'].append('auto_start_backend')
                    return True
    ...
    def print_diagnostic_report(self, diagnostics: Dict):
        ...
        status_emoji = "🟢" if ai_analysis['confidence_score'] > 0.8 else "🟡" if ai_analysis['confidence_score'] > 0.5 else "🔴"
        ...
        if diagnostics['backend']['status'] == 'healthy' and diagnostics['frontend']['status'] == 'healthy':
...
def main():
    ...
    try:
        ...
        if (results['backend']['status'] == 'healthy' and 
